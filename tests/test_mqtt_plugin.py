...
class MqttPluginTests(unittest.TestCase):
    def setUp(self):
        ...
        mqtt_src = source_root / "plugins" / "mqtt"
        mqtt_dst = self.plugins_root / "mqtt"
        mqtt_dst.mkdir(parents=True, exist_ok=True)
        mqtt_dst.joinpath("__init__.py").write_text(mqtt_src.joinpath("__init__.py").read_text(encoding="utf-8"), encoding="utf-8")
        mqtt_dst.joinpath("actions.py").write_text(mqtt_src.joinpath("actions.py").read_text(encoding="utf-8"), encoding="utf-8")
    ...
    def test_mqtt_actions_visible_after_refresh(self):
        ...
        self.assertIn("mqtt.MqttPublish", full_names)
        self.assertIn("mqtt.MqttSubscribeOnce", full_names)
        self.assertIn("mqtt.MqttRequestReply", full_names)
        self.assertIn("mqtt.MqttCheckConnection", full_names)
